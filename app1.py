                ...
                if old_page and new_page:
                    ...
                    if old_code and new_code:
                        ...
                        if "impact_text" not in st.session_state:
                            st.session_state.impact_text = safe_generate(f"Analyze this code diff and explain the impact with slightly less than mid sized content:\n\n{safe_diff}")
                        if "rec_text" not in st.session_state:
                            st.session_state.rec_text = safe_generate(f"As a senior engineer, suggest improvements for this diff with slightly less than mid sized content:\n\n{safe_diff}")
                        if "risk_text" not in st.session_state:
                            raw_risk = safe_generate(f"Assess the risk of each change in this code diff with severity tags (Low, Medium, High) and keep it slightly less:\n\n{safe_diff}")
